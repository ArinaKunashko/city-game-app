    ...
    <div class="flex justify-center items-center vh100 ">
        <div class="flex-col rounded-2xl shadow-lg bg-white max-width ">
            <div class="flex flex-row prose m-4 justify-between text-black">
                <h4 v-if="getLastAuthor === 'bot'" class="text-base font-normal">Сейчас ваша очередь
                </h4>
                <h4 v-else class="text-base font-normal">Сейчас очередь
                    соперника</h4>
                <count-component> </count-component>
            </div>
            <div class="bg-gray-200 h-1 " style="width: 45%"></div>
            <div id="chat" :class="{ 'justify-center': this.getMessages.length === 0 }"
                ...
                <p v-if="this.getMessages.length === 0" class=" font-normal text-gray-400 w-max my-36 mt-2 mb-1 p-2">
                    Первый участник вспоминает города...
                </p>
                <ul class="w-full">
                    <li v-for="(message, idx) in this.$store.state.messages" :key="idx" :class="message.author">
                        <message-text :message="message"> </message-text>
                    </li>
                </ul>
            </div>
            <p class="text-gray-400 text-center mb-0 text-sm"> Всего перечислено городов: {{ getMessages.length }}</p>
            <div class="bg-gray-100 flex flex-row border-gray items-center border rounded h-12 mx-4 mt-7 ">
                ...
                <div class="flex flex-row">
                    <button id="self" :disabled="getLastAuthor === 'client'" :class="{
                        'flex items-center justify-center h-8 w-8 mr-2 rounded-md bg-purple-500 hover:purple-400 text-white focus:outline-none': getLastAuthor === 'bot',
                        'flex items-center justify-center h-8 w-8 mr-2 rounded-md bg-gray-400 hover:purple-400 text-white focus:outline-none': getLastAuthor === 'client'
                    }" @click="sendMessage">
                        <svg class="w-5 h-5 transform rotate-45 -mr-1 -mt-1" fill="none" stroke="currentColor"
...
export default {
    ...
    methods: {
        ...
        sendMessage() {
            ...
            if (this.getMessages.length > 0 && !message.startsWith(this.getLastLetter)) {
            ...
            this.message = ''
            setTimeout(() => {
                this.addMessage({ text: this.getCity(this.getLastLetter), author: 'bot' })
